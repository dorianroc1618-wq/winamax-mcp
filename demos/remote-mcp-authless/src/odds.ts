// OddsPapi v4 normalized catalogue.
// Never infer a market label from its ID.
...
export class OddsClient {
  ...
  async get(
    ...
    if (!this.env.ODDSPAPI_API_KEY) {
      throw new ProviderError("Secret OddsPapi absent.");
    }

    const previous =
      this.queues.get(endpoint) ?? Promise.resolve();
    ...
    const pending = previous
      ...
      .then(async () => {
        for (let attempt = 0; attempt < 3; attempt++) {
          const wait =
            (this.next.get(endpoint) ?? 0) - Date.now();
          ...
          if (response.status === 429) {
            if (attempt >= 2) {
              throw new ProviderError(
                "OddsPapi: limite de requêtes, réessayer plus tard.",
              );
            }
          ...
          if (!response.ok) {
            throw new ProviderError(
              `OddsPapi ${endpoint}: HTTP ${response.status}.`,
            );
          }
          ...
          try {
            ...
            throw new ProviderError(
              `OddsPapi ${endpoint}: réponse JSON invalide.`,
            );
          }
        }

        throw new ProviderError(
          `OddsPapi ${endpoint}: nombre maximal de tentatives atteint.`,
        );
      });
    ...
    if (!cached || cached.expires < Date.now()) {
      const value = this.get("markets", {
        ...
        if (!Array.isArray(rows)) {
          throw new ProviderError(
            "Catalogue des marchés invalide.",
          );
        }

        return new Map<string, Dict>(
          rows.map((row: Dict) => [
            String(row.marketId),
            row,
          ]),
...
export function fixtureView(
  ...
  return {
    ...
    competition:
      f.tournamentName ?? null,

    tournamentId:
      f.tournamentId ?? null,

    participants: [
      f.participant1Name ?? null,
      f.participant2Name ?? null,
    ],
...
function quote(
  ...
  return {
    ...
    bookmakerTimestamp:
      player.bookmakerChangedAt ?? null,
...
export function normalizeOdds(
  ...
  if (min > max) {
    throw new ProviderError(
      "minOdds doit être inférieur ou égal à maxOdds.",
    );
  }
  ...
  if (!books[WINAMAX]) {
    warnings.push(
      "Aucune cote Winamax retournée pour cet événement.",
    );
  }
  ...
  return {
    ...
    rlm: {
      ...
      reason:
        "Aucune donnée de répartition des mises/tickets; un mouvement de cote ne suffit pas.",
    },
  };
}
...
export class Scanner {
  ...
  async search(
    ...
    return {
      ...
      note:
        "countryOrCategory reprend la catégorie du fournisseur; country reste null pour les catégories sans pays reconnu. Les filtres de cotes s'appliquent avec includeMarkets=true.",
    };
  }
}
